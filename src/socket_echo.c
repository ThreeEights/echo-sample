...
int main(void)
{
	...
	static struct wifi_connect_req_params cnx_params = {
		.ssid = CONFIG_ECHO_SAMPLE_WIFI_SSID,
		.ssid_length = 0,
		.psk = CONFIG_ECHO_SAMPLE_WIFI_PSK,
		.psk_length = 0,
		.channel = 0,
		.security = WIFI_SECURITY_TYPE_PSK,
	};

	cnx_params.ssid_length = strlen(CONFIG_ECHO_SAMPLE_WIFI_SSID);
	cnx_params.psk_length = strlen(CONFIG_ECHO_SAMPLE_WIFI_PSK);
